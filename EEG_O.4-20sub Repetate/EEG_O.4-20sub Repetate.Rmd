...
# Normality 
Repetat_merged_Neg %>%
  dplyr::sample_n(5000) %>%                                                # randomly sample 5000 rows because shapiro.test with with max 5000
...
```{r comp_gender2, fig.height=14}
stat.test <-
  Repetat_merged_gender %>%
  group_by(Stimulus.type) %>%
  rstatix::t_test(SAM_Resp ~ Cond_Gender, p.adjust.method = "holm") %>%              
...
if(nrow(stat.test) > 0){
  p <- 
    p + stat_pvalue_manual(stat.test, label = "p.adj.signif", 
                          vjust = 0.9, tip.length = 0.01,
                           y.position = seq(max(Repetat_merged_gender$SAM_Resp, na.rm = TRUE)*1.01, max(Repetat_merged_gender$SAM_Resp, na.rm = TRUE)*7, 
