...
```{r}
my_ggbetweenstats_oxt <- function(data, title, x, y, outlier.label, xlab, ylab, 
                              outlier.tagging = FALSE, results.subtitle = FALSE, 
  ...
  if(centrality.label.args){
    centrality.label.args <- list(size = 3, nudge_x = 0.4, segment.linetype = 5, fill = "white")
  }else{
    centrality.label.args <- list(size = 0, nudge_x = 0.2, segment.linetype = 0, alpha = 0) # very hacky way of not showing label
  }
...
df_ox_pre <- 
  df_sam_ox_long %>%
    ...
    filter(n() > 1) %>%                      # exclude incomplete
    ungroup()
df_ox_pre_id <- df_ox_pre$id

plot_oxt_pre1_np <- df_memo_ox_m1_plot %>% 
  filter(Condition %in% c("VR odor", "VR"),
         ID %in% df_ox_pre_id) %>% 
  ggbetweenstats(
    x = Condition,
    y = oxitocina_normalizata_PRE,
    type = "np"
  )  
# fast_tiffsave(plot_oxt_pre1_np, path = here::here("Results"), width = 8, height = 6.5)

plot_oxt_pre2_np <- df_memo_ox_m1_plot %>%  
  filter(Condition %in% c("VR odor", "VR social"),
         ID %in% df_ox_pre_id) %>% 
  ggbetweenstats(
    x = Condition,
    y = oxitocina_normalizata_PRE,
...
# fast_tiffsave(plot_oxt_pre2_np, path = here::here("Results"), width = 8, height = 6.5)

plot_oxt_pre3_np <- df_memo_ox_m1_plot %>% 
  filter(Condition %in% c("VR", "VR social"),
         ID %in% df_ox_pre_id) %>% 
  ggbetweenstats(
    x = Condition,
    y = oxitocina_normalizata_PRE,
...
# fast_tiffsave(plot_oxt_pre3_np, path = here::here("Results"), width = 8, height = 6.5)

#
plot_oxt_pre1_p <- df_memo_ox_m1_plot %>% 
  filter(Condition %in% c("VR odor", "VR"),
         ID %in% df_ox_pre_id) %>% 
  ggbetweenstats(
    x = Condition,
...
# fast_tiffsave(plot_oxt_pre1_p, path = here::here("Results"), width = 8, height = 6.5)

plot_oxt_pre2_p <- df_memo_ox_m1_plot %>%  
  filter(Condition %in% c("VR odor", "VR social"),
         ID %in% df_ox_pre_id) %>% 
  ggbetweenstats(
    x = Condition,
...
# fast_tiffsave(plot_oxt_pre2_p, path = here::here("Results"), width = 8, height = 6.5)

plot_oxt_pre3_p <- df_memo_ox_m1_plot %>% 
  filter(Condition %in% c("VR", "VR social"),
         ID %in% df_ox_pre_id) %>% 
  ggbetweenstats(
    x = Condition,
...
# fast_tiffsave(plot_oxt_pre3_p, path = here::here("Results"), width = 8, height = 6.5)

# --
plot_oxt_pres <- df_memo_ox_m1_plot %>% 
  filter(Condition != "odor",
         ID %in% df_ox_pre_id) %>% 
  ggbetweenstats(
    x = Condition,
...
# fast_tiffsave(plot_oxt_pres, path = here::here("Results"), width = 10, height = 6.5)

# ---
# DIF
# plot_oxt_dif <- df_memo_ox_m1_plot %>%
#   filter(Condition != "odor", oxitocina_normalizata_DIF < 2) %>% 
#   my_ggbetweenstats(
#     x = Condition,
#     y = oxitocina_normalizata_DIF,
#     outlier.label = ID,
#     title = "",
#     xlab = "",
#     ylab = "OXT DIF"#,
...
# fast_tiffsave(plot_oxt_dif, path = here::here("Results"), width = 8, height = 6.5)
...
#                  "D" = 8:13,
#                  "E" = 14:20)
# mycolors <- c("grey40", "grey60", "grey80", "grey100") # c("lightblue", "lightgreen", "lightpink", "yellow")        
# mylegend <- "bla"
# # Weight Mat
# net_stud$graph %>% 
#   kableExtra::kbl(caption = "Net Stud Weights", digits = 2) %>%
#   kableExtra::kable_classic(full_width = F, html_font = "Times New Roman")
# # Print
# tiff(filename = "Student Network - 1grey600.tiff", width = 8, height = 8, units = "in", res = 600)
# plot(net_stud, layout = "spring", border.color = "black", 
#      groups = mygroups, color = mycolors, edge.color = "black",  
#      legend = TRUE, legend.mode = "groups", 
#      vsize = 7, node.width = .8, node.height = .8, legend.cex = .7,
...
df_memo_ox_m1_plot %>%
  ...
  estimateNetwork(default = "cor",  # "EBICglasso"
                  corMethod = "cor", 
                  corArgs = list(method = "pearson", use = "pairwise.complete.obs")) %>% 
  plot(layout = "groups", vsize = 6, border.color = "black")
