...
file <- "30.03.2021 Raspunsuri chestionar inscriere PA4.xlsx"
...
TR_new <- c("07pilotA10", "08pilotA10", "10pilotA10", "BpilotA10", "adrianavisanpsiho@yaho.com")
...
## T test TR (treatment both old and new)

```{r ttest_TR}
data_TR <- 
  data %>%
  dplyr::filter(ID %in% c(TR, TR_new))
...
# Plot
bxp_TRn <- 
  ggpubr::ggpaired(data_TRn, x = "PrePost", y = "APS_Total", id = "ID",
                   order = c("Pre", "Post"),
                   ylab = "APS_Total", xlab = "Cond")
...
bxp_TRn + 
  ggpubr::stat_pvalue_manual(stat_test_TRn, tip.length = 0) +
  labs(subtitle = get_test_label(stat_test_TRn, detailed= TRUE))  
```
