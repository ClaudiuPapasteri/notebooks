...
extract_botrec <- function(file, path) {
  ...
  frames_df_wide <- 
    ...
    tidyr::fill(previousGameState_f, newGameState_f, timeStamp, .direction = "down") 
...
bla <-
  bla %>%
  dplyr::mutate(
    markerPosition_check_x = dplyr::coalesce(StartMarker_x, FlagMarker_x, ObjectMarker_x),
    markerPosition_check_y = dplyr::coalesce(StartMarker_y, FlagMarker_y, ObjectMarker_y),
    markerPosition_check_z = dplyr::coalesce(StartMarker_z, FlagMarker_z, ObjectMarker_z))

bla_check <- 
  bla %>%
  dplyr::select(c(
    "newGameState_f", 
    "markerPosition_check_x", "markerPosition_x_demoimit", 
    "markerPosition_check_z", "markerPosition_z_demoimit")) %>% 
...
all.equal(bla_check[["markerPosition_check_x"]], bla_check[["markerPosition_x_demoimit"]])
all.equal(bla_check[[ "markerPosition_check_z"]], bla_check[["markerPosition_z_demoimit"]])
# -------------------------------------------------------------------------
###########################################################################################
