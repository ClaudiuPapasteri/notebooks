...
file_name <- paste0("ieeg_STAD-inregistrari_", Sys.Date())
...
folder_clean_botrec <- file.path(folder, "Clean_Sessions_ieeg")
...
pattern_files_botrec <- "^(?=.*TestSession|config|Config_)(?=.*botrec)"                        # contains "TestSession" or "config", contains ".botrec"
pattern_files_botrec_noclean <- "^(?=.*TestSession|config|Config_)(?!.*_clean)(?=.*botrec)"    # only .botrec files that were not cleaned (do not have "_clean")
...
all.equal(check_file_count1, check_file_count2, check.names = FALSE)   # new check counts
...
saveRDS(data_clean_output, file = file.path(folder, "data_clean_output_ieeg.RDS"))
