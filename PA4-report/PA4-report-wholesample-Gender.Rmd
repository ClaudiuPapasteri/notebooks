...
cat("## DG - PTSD TR")
dg_united_wide %>%
  dplyr::filter(ID %in% ids_ptsd) %>%  dplyr::filter(ID %in% fem_ids) %>%
...
cat("## DG - Burnout TR")
dg_united_wide %>%
  dplyr::filter(ID %in% ids_burn) %>%  dplyr::filter(ID %in% fem_ids) %>%
  dplyr::filter(Cond == "TR") %>%
  func_t_box(., ind = "ID", pre_var = "DG_Total_Pre", post_var = "DG_Total_Post")
cat("## DG - Burnout CTRL")
dg_united_wide %>%
...
dg_united_wide %>%
  dplyr::filter(ID %in% ids_normal) %>%   dplyr::filter(ID %in% fem_ids) %>%
  dplyr::filter(Cond == "TR") %>%
  func_t_box(., ind = "ID", pre_var = "DG_Total_Pre", post_var = "DG_Total_Post")
...
# Merge gender
scale_united_wide <- dplyr::left_join(scale_united_wide, gender_match, by = "ID")
```
